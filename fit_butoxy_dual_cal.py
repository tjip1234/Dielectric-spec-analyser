"""
Dual-Reference Calibration (Water + Isopropanol) for 2-Butoxyethanol Analysis
...
4. Apply mapping to 2-Butoxyethanol S1P to extract permittivity.
...
def fit_debye_parameters(freq_hz, eps_prime, eps_double_prime):
    ...
    # Initial guesses for 2-Butoxyethanol
    # Expected: eps_s ~ 9-13, eps_inf ~ 2-3, tau ~ 100-300 ps
    x0 = [10.0, 2.5, 200e-12] # eps_s, eps_inf, tau
    bounds = [(2, 30), (1, 10), (1e-13, 1e-9)]
...
def plot_results(calibration, target_result, target_fit, output_path):
    ...
    ax.plot(np.real(target_result['y_target']), np.imag(target_result['y_target']), 'r-', label='Butoxy (Target)')
    ...
    ax.set_title(f"Butoxy ε' Fit (εs={target_fit['eps_s']:.2f})")
    ax.set_xlabel("Frequency (GHz)")
    ...
    ax.set_title(f"Butoxy ε'' Fit (τ={target_fit['tau']*1e12:.1f}ps)")
    ax.set_xlabel("Frequency (GHz)")
    ax.set_ylabel("Imaginary Permittivity")
    ax.legend()
    ax.grid(True, alpha=0.3)
    
    plt.suptitle(f"2-Butoxyethanol Dual-Cal Analysis (Water+Iso)\nR²={target_fit['r2']:.4f}", fontsize=14)
...
def main():
    print("="*80)
    print("2-BUTOXYETHANOL DUAL-CALIBRATION ANALYSIS")
    ...
    try:
        ...
        # 2-Butoxyethanol
        butoxy_path = Path("Database/2-Butoxyethanol/2-Butoxyethanol.s1p")
        if not butoxy_path.exists(): butoxy_path = Path("compounds/2-Butoxyethanol.s1p")
        s1p_butoxy = load_s1p_file(butoxy_path)
    ...
    # 4. Apply to 2-Butoxyethanol
    print("Applying Calibration to 2-Butoxyethanol...")
    butoxy_result = apply_dual_calibration(cal, s1p_butoxy)
    
    # 5. Fit Debye
    print("Fitting Debye Model...")
    fit = fit_debye_parameters(butoxy_result['freq_hz'], 
                               butoxy_result['eps_prime'], 
                               butoxy_result['eps_double_prime'])
    
    print("\nRESULTS:")
    print(f"Static Permittivity (εs): {fit['eps_s']:.2f} (Expected ~9-13)")
    print(f"High-Freq Permittivity (ε∞): {fit['eps_inf']:.2f} (Expected ~2-3)")
    ...
    plot_results(cal, butoxy_result, fit, output_dir / "butoxy_dual_cal.png")
