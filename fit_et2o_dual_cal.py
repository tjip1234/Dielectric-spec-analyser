"""
Dual-Reference Calibration (Water + Isopropanol) for Diethyl Ether Analysis
...
4. Apply mapping to Diethyl Ether S1P to extract permittivity.
...
def fit_debye_parameters(freq_hz, eps_prime, eps_double_prime):
    ...
    # Initial guesses for Diethyl Ether
    x0 = [4.3, 1.8, 3e-12] # eps_s, eps_inf, tau
...
def plot_results(calibration, et2o_result, et2o_fit, output_path):
    """Plot the results"""
    ...
    ax.plot(np.real(et2o_result['y_target']), np.imag(et2o_result['y_target']), 'r-', label='Et2O (Target)')
    ...
    # 3. Extracted Et2O Permittivity & Fit
    ax = axes[1, 0]
    ...
    ax.set_title(f"Et2O ε' Fit (εs={et2o_fit['eps_s']:.2f})")
    ax.set_xlabel("Frequency (GHz)")
    ...
    plt.suptitle(f"Diethyl Ether Dual-Cal Analysis (Water+Iso)\nR²={et2o_fit['r2']:.4f}", fontsize=14)
...
def main():
    print("="*80)
    print("DIETHYL ETHER DUAL-CALIBRATION ANALYSIS")
    ...
    try:
        ...
        if not et2o_path.exists(): et2o_path = Path("compounds/diethyl-ether.s1p")
    ...
    # 4. Apply to Diethyl Ether
    print("Applying Calibration to Diethyl Ether...")
    et2o_result = apply_dual_calibration(cal, s1p_et2o)
    ...
    plot_results(cal, et2o_result, fit, output_dir / "et2o_dual_cal.png")
