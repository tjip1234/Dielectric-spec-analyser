"""
Calibrate S11->Permittivity using NPL Isopropanol data, then apply to Water.
...
4. Load your water S1P measurement
5. Apply the same mapping to get water's epsilon', epsilon''
6. Fit Debye model to water's extracted permittivity
"""
...
def single_debye(f, eps_s, eps_inf, tau):
    """Wrapper for curve_fit"""
    return debye_model(f, eps_s, eps_inf, tau)
...
def load_literature_water():
    ...
    # Columns: Freq_GHz, eps_prime_25C, eps_double_prime_25C, eps_prime_20C, eps_double_prime_20C
    freq_ghz = data[:, 0]
    eps_prime_25C = data[:, 1]
    eps_double_prime_25C = data[:, 2]
    eps_prime_20C = data[:, 3]
    eps_double_prime_20C = data[:, 4]
    
    return {
        'freq_ghz': freq_ghz,
        'eps_prime_25C': eps_prime_25C,
        'eps_double_prime_25C': eps_double_prime_25C,
        'eps_prime_20C': eps_prime_20C,
        'eps_double_prime_20C': eps_double_prime_20C
    }
...
def fit_debye_parameters(freq_hz, eps_prime, eps_double_prime):
    ...
    # Initial guesses for Water
    x0 = [78.0, 5.0, 8e-12] # eps_s, eps_inf, tau
    bounds = [(10, 100), (1, 20), (1e-13, 1e-10)]
...
def plot_results(npl_iso, calibration, water_result, water_fit, lit_water, output_path):
    """Plot the results"""
    ...
    # 3. Extracted Water Permittivity
    ax = axes[1, 0]
    ax.plot(water_result['freq_ghz'], water_result['eps_prime'], 'b-', label="Water ε' (Extracted)")
    ax.plot(water_result['freq_ghz'], water_result['eps_double_prime'], 'r-', label="Water ε'' (Extracted)")
    
    # Plot Literature Data if available
    if lit_water:
        ax.plot(lit_water['freq_ghz'], lit_water['eps_prime_20C'], 'c--', alpha=0.7, label="Lit 20°C ε'")
        ax.plot(lit_water['freq_ghz'], lit_water['eps_double_prime_20C'], 'm--', alpha=0.7, label="Lit 20°C ε''")
        
    ax.set_title("Extracted: Water Permittivity vs Literature")
    ax.set_xlabel("Frequency (GHz)")
    ax.set_ylabel("Permittivity")
    ax.legend()
    ax.grid(True, alpha=0.3)

    # 4. Empty or Admittance Comparison
    ax = axes[1, 1]
    ax.plot(calibration['freq_hz']/1e9, np.abs(calibration['y_iso']), 'k-', label="|Y_iso|")
    ax.plot(water_result['freq_ghz'], np.abs(water_result['y_target']), 'b--', label="|Y_water|")
    ...
    ax = axes[2, 0]
    freq_smooth = np.linspace(water_result['freq_hz'].min(), water_result['freq_hz'].max(), 100)
    ep_fit, edp_fit = debye_model(freq_smooth, water_fit['eps_s'], water_fit['eps_inf'], water_fit['tau'])
    
    ax.plot(water_result['freq_ghz'], water_result['eps_prime'], 'bo', alpha=0.5, label="Data")
    ax.plot(freq_smooth/1e9, ep_fit, 'k--', linewidth=2, label="Debye Fit")
    
    if lit_water:
        ax.plot(lit_water['freq_ghz'], lit_water['eps_prime_20C'], 'c--', alpha=0.5, label="Lit 20°C")
        
    ax.set_title(f"Water ε' Fit (εs={water_fit['eps_s']:.1f})")
    ax.set_xlabel("Frequency (GHz)")
    ...
    ax = axes[2, 1]
    ax.plot(water_result['freq_ghz'], water_result['eps_double_prime'], 'ro', alpha=0.5, label="Data")
    ax.plot(freq_smooth/1e9, edp_fit, 'k--', linewidth=2, label="Debye Fit")
    
    if lit_water:
        ax.plot(lit_water['freq_ghz'], lit_water['eps_double_prime_20C'], 'm--', alpha=0.5, label="Lit 20°C")
        
    ax.set_title(f"Water ε'' Fit (τ={water_fit['tau']*1e12:.1f}ps)")
    ax.set_xlabel("Frequency (GHz)")
    ax.set_ylabel("Imaginary Permittivity")
    ax.legend()
    ax.grid(True, alpha=0.3)
    
    plt.suptitle(f"Water Debye Analysis via Isopropanol Calibration\nR²={water_fit['r2']:.4f}", fontsize=14)
...
def main():
    print("="*80)
    print("WATER DEBYE ANALYSIS (Isopropanol Calibration)")
    print("="*80)
    ...
    # 4. Load Water S1P
    print("Loading Water S1P...")
    water_path = Path("Database/Water/DI-water.s1p")
    if not water_path.exists():
        water_path = Path("compounds/DI-water.s1p")
        
    if not water_path.exists():
        print("Error: Could not find water S1P file")
        return
    s1p_water = load_s1p_file(water_path)
    
    # 5. Apply Calibration
    print("Applying Calibration to Water...")
    water_result = apply_calibration(cal, s1p_water)
    
    # 6. Fit Debye
    print("Fitting Debye Model...")
    fit = fit_debye_parameters(water_result['freq_hz'], 
                               water_result['eps_prime'], 
                               water_result['eps_double_prime'])
    
    print("\nRESULTS:")
    print(f"Static Permittivity (εs): {fit['eps_s']:.2f} (Expected ~78-80)")
    print(f"High-Freq Permittivity (ε∞): {fit['eps_inf']:.2f} (Expected ~4-6)")
    print(f"Relaxation Time (τ): {fit['tau']*1e12:.2f} ps (Expected ~8-9 ps)")
    print(f"Relaxation Freq: {fit['f_relax']/1e9:.2f} GHz")
    print(f"Fit Quality (R²): {fit['r2']:.4f}")
    
    # 7. Load Literature Data
    print("Loading Literature Water Data...")
    lit_water = load_literature_water()

    # 8. Plot
    output_dir = Path("demo_results")
    output_dir.mkdir(exist_ok=True)
    plot_results(npl_iso, cal, water_result, fit, lit_water, output_dir / "water_debye_calibrated.png")
